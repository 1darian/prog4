...
const csrfRoutes = require('../../src/routes/csrf');
...
describe('Seguridad: CSRF (Cross-Site Request Forgery)', () => {
  ...
  test('❌ DEBE FALLAR: Los tokens CSRF deben ser únicos por sesión', async () => {
    ...
    if (tokenResponse.status !== 200) {
      console.error('Token Response Error:', tokenResponse.status, tokenResponse.body);
    }
    ...
    if (token1 === token2) {
      throw new Error(`Tokens are identical! Token1: ${token1}, Token2: ${token2}`);
    }
